...
#define CS_LOW()		SPI_PORT &= ~(1 << CS)		 /* Set CS (Chip Select = Slave Select (SS)) low */
#define	CS_HIGH()		SPI_PORT |=  (1 << CS)		/* Set CS high */
...
//#define CS_LOW()		PORTD.OUTCLR = PIN5_bm	/* Set MMC_CS = low */
//#define	CS_HIGH()		PORTD.OUTSET = PIN5_bm	/* Set MMC_CS = high */
//#define MMC_CD			1						/* Test if card detected.   yes:true, no:false, default:true */
//#define MMC_WP			0						/* Test if write protected. yes:true, no:false, default:false */
//#define	FCLK_SLOW()		SPI0.CTRLA = SPI_PRESC_DIV128_gc | SPI_MASTER_bm | SPI_ENABLE_bm; /* Set SPI clock for initialization (100-400kHz) *///
//#define	FCLK_FAST()		SPI0.CTRLA = SPI_PRESC_DIV4_gc | SPI_CLK2X_bm | SPI_MASTER_bm | SPI_ENABLE_bm; /* Set SPI clock for read/write (20MHz max) */
...
void power_on (void)
{
	...
	//Oude code:
	// Init SPI interface: see datasheet 28.3.1
	/*'
	PA4: MOSI (output), PA5: MISO (input), PA6: SCK (output)
	PORTA.DIR |= (PIN4_bm | PIN6_bm);
	PORTMUX.SPIROUTEA = PORTMUX_SPI0_DEFAULT_gc;		// Select SPI pins PA4/PA5
	
	PORTD.DIR |= PIN5_bm;								// PORTD pin 5 = SS//
	PORTD.OUTSET = PIN5_bm;								// deselect SD card
	
	SPI0.CTRLA |= SPI_MASTER_bm;						// SPI master
	SPI0.CTRLA |= SPI_PRESC_DIV128_gc;					// slowest clock to start with
	SPI0.CTRLB |= SPI_SSD_bm;							// we control our own SS line, so disable auto SS by SPI0
	
	SPI0.INTCTRL = 0x00;
	SPI0.INTFLAGS = 0x00;
	
	SPI0.CTRLA |= SPI_ENABLE_bm;						// enable SPI interface		
	...
	*/
	...
	SPI_PORT |= (1<<CS); 				//hoog maken CS  
	
	/* Enable SPI, Master, set clock rate fck/16 */
	SPCR |= ((1<<SPE) | (1<<MSTR));
	

	FCLK_SLOW();						//low speed SPI, div 128
	
	//nog niet zeker of interrupts bij onze microcontroller nog moet worden uitgezet
